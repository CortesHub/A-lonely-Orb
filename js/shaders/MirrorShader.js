...
THREE.MirrorShader = {
	...
	fragmentShader: [
		...
		"void main() {",
			...
			// LIGHT
			"vec3 lightDirection = normalize( lightPosition.xyz - vWorldPosition );",
			"float c = 0.1 + max( 0.0, dot(vNormal, lightDirection) ) * 1.;",
			"vec4 intensity = vec4(c, c, c, 1.);",	
		
			"vec4 mask = texture2D(maskSampler, texCoord);",

			"vec4 color = texture2D(diffuseSampler, texCoord);",
			"vec4 color2 = texture2D(diffuseSampler, texCoord);",
			"vec4 mColor = texture2DProj(mirrorSampler, mirrorCoord);",
			"mColor = vec4(blendOverlay(mirrorColor.r, mColor.r), blendOverlay(mirrorColor.g, mColor.g), blendOverlay(mirrorColor.b, mColor.b), c);",
			"color = mix(mColor*mask, color, clamp(dot(vNormal, normalize(eyeVec)/2.0+0.4), 0.4, 0.9));",
		
			"gl_FragColor = vec4(color2 * intensity );",
...
THREE.Mirror = function (renderer, camera, options) {
	...
	var mirrorShader = THREE.MirrorShader;
